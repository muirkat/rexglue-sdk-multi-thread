...
- **`rexglue` codegen is single-threaded.** The analysis phases (`phase_*.cpp`) run in a fixed sequence on one thread, and `ProjectRecompiler` processes the entrypoint and each DLL module sequentially — there is no thread pool, `std::async`, or parallel-for anywhere in `src/codegen/`. The per-module work (analysis + C++ emission) is naturally independent, which is the obvious axis to parallelize; if you do, note that codegen briefly stands up a `Runtime` in tool mode per binary, so shared global state (e.g. `Runtime::instance()`, logging, the template registry) is what to audit first.
