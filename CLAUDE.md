...
- **`rexglue` codegen analysis is single-threaded; C++ emission is parallel.** The analysis phases (`phase_*.cpp`) run in a fixed sequence on one thread, and `ProjectRecompiler` processes the entrypoint and each DLL module sequentially. The hot path — translating every discovered function to C++ — runs in parallel: `CodegenWriter::write()` calls `emitFunctionsParallel()` (`codegen_writer.cpp`), which fans `FunctionNode::emitCpp()` across worker threads via an atomic work cursor. This is safe because `emitCpp()` is `const` and reads only shared-const state (`BinaryView`, `RecompilerConfig`, `FunctionGraph`, and the read-only `ExportResolver`); each call builds its own `BuilderContext`. Results are collected into a per-index vector and the file assembly/size-splitting stays serial, so output is **byte-identical regardless of thread count**. Worker count is `codegen_threads` (cvar, `0` = autodetect via `hardware_concurrency()`). Two invariants to preserve if you touch this: graph mutation (e.g. the rexcrt `setName` rename and import/rexcrt filtering) must stay **before** the parallel loop, and nothing in the emit path may write through those const references. The remaining serial axis — per-module analysis — is independent per binary but each module briefly stands up a `Runtime` in tool mode, so audit shared global state (`Runtime::instance()`, the template registry) before parallelizing it.
