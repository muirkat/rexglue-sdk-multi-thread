...
bool CodegenWriter::write(bool force) {
  ...
  // Generate {project}_common.h: the STABLE shell of init.h (config defines + SDK
  // includes + the DECLARE/DEFINE_REX_FUNC macros) WITHOUT the ~per-function
  // DECLARE_REX_FUNC list. Recomp TUs include this plus a slim per-file decls header
  // (below) instead of the global init.h, so adding/removing one function no longer
  // churns a header textually included by every TU -- which would cold-miss ccache and
  // force a full rebuild. Rendered from the same init_h template with an empty function
  // list (init_h.inja references `functions` only in the DECLARE_REX_FUNC loop).
  {
    auto commonData = tmplData;
    commonData["functions"] = nlohmann::json::array();
    out = renderWithJson(registry, "codegen/init_h", commonData);
    SaveCurrentOutData(fmt::format("{}_common.h", projectName));
  }
  ...
  // Each {project}_recomp.N.cpp includes a slim {project}_recomp.N.decls.h holding only
  // the DECLARE_REX_FUNC lines that file actually references (its own functions + their
  // direct call / tail-call targets), instead of the global init.h. A one-function
  // manifest edit then changes only the decls headers of the files that call it, so
  // unchanged TUs stay ccache-hot and the rebuild collapses from ~all-148 to a handful.
  // (init.cpp still includes the full init.h to build the address->function table.)
  size_t currentFileBytes = 0;
  std::vector<const FunctionNode*> currentFileFns;
  // Resolve a node to the exact symbol the emitter/init.h uses (mirrors
  // buildTemplateData lines ~63-73): entry point -> "xstart", rexcrt -> renamed,
  // empty -> sub_<addr>. Using raw name() would miss the entry point (empty name,
  // emitted as the weak-alias symbol "xstart") and any unnamed callee.
  auto nameOf = [&](const FunctionNode* fn) -> std::string {
    auto it = rexcrtByAddr.find(static_cast<uint32_t>(fn->base()));
    if (it != rexcrtByAddr.end()) return it->second;
    if (fn->base() == analysisState().entryPoint) return "xstart";
    if (!fn->name().empty()) return fn->name();
    return fmt::format("sub_{:08X}", fn->base());
  };
  auto flushDeclsHeader = [&]() {
    std::set<std::string> decls;
    for (const FunctionNode* fn : currentFileFns) {
      decls.insert(nameOf(fn));
      for (const auto& e : fn->calls())
        if (const FunctionNode* c = e.target.asFunction()) decls.insert(nameOf(c));
      for (const auto& e : fn->tailCalls())
        if (const FunctionNode* c = e.target.asFunction()) decls.insert(nameOf(c));
    }
    std::string h = fmt::format("// Generated by rexglue - DO NOT EDIT\n#pragma once\n"
                                "#include \"{}_common.h\"\n",
                                projectName);
    for (const std::string& n : decls)
      h += fmt::format("DECLARE_REX_FUNC({});\n", n);
    pendingWrites.emplace_back(
        fmt::format("{}_recomp.{}.decls.h", projectName, cppFileIndex), std::move(h));
    currentFileFns.clear();
  };

  println("#include \"{}_recomp.{}.decls.h\"\n", projectName, cppFileIndex);
  ...
  for (size_t i = 0; i < functions.size(); i++) {
    ...
    currentFileFns.push_back(functions[i]);
  }
